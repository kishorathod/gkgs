...
export class LearningModuleEngine {
  ...
  /**
   * Master render method: Renders all 15 learning blocks dynamically
   */
  renderAll() {
  ...
  renderHero(hero) {
    ...
    if (badgeEl && this.topicData.subject) {
      badgeEl.textContent = `📚 ${this.topicData.subject.toUpperCase()} — TOPIC MODULE`;
    }
  ...
  switchSubTab(subtabId) {
    ...
    const paneMap = {
      'overview': `${this.subject}-subtab-overview`,
      'timeline': `${this.subject}-subtab-timeline`,
      'notes': `${this.subject}-subtab-notes`,
      'pyqs': `${this.subject}-subtab-pyqs`,
      'quiz': `${this.subject}-subtab-quiz`,
      'flashcards': `${this.subject}-subtab-flashcards`,
      'revisionsheet': `${this.subject}-subtab-revisionsheet`
    };
    ...
    const targetPaneId = paneMap[subtabId] || `${this.subject}-subtab-${subtabId}`;
  ...
  // ── 4. Subject Visual Pane (Timeline / Map / Diagram / Articles) ─────────
  renderVisualPane(data) {
    ...
    // Strictly render topic-specific timeline items (e.g. 1857 Revolt events)
    let html = `
      ...
      <div class="glass-card" style="padding: 20px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 16px;">
          ${q.options.map((opt, i) => {
            ...
            let optClass = 'btn btn-secondary';
            ...
            return `<button class="${optClass}" data-opt="${opt}" ${userAnswer !== null ? 'disabled' : ''} style="${style}">${opt}</button>`;
          }).join('')}
        </div>
  ...
  renderFlashcardCard(pane) {
    ...
    pane.innerHTML = `
      <div>
        <div id="${this.subject}-fc-box" class="glass-card" style="min-height: 180px; padding: 24px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; cursor: pointer; border: 1px solid var(--accent-purple); margin-bottom: 16px;">
          <span style="font-size: 11px; color: var(--text-muted); margin-bottom: 8px;">Card ${this.flashcardIdx + 1} of ${cards.length} (Click to flip)</span>
          <div style="font-size: 16px; font-weight: 700; color: ${this.flashcardFlipped ? 'var(--accent-emerald)' : 'var(--text-main)'};">
        ...
        <div style="display: flex; justify-content: space-between;">
          ...
          <button class="btn btn-primary" id="${this.subject}-fc-next" ${this.flashcardIdx === cards.length - 1 ? 'disabled' : ''}>Next →</button>
        </div>
      </div>
    `;
  ...
  renderRelatedTopicsPane(data) {
    ...
    if (prevBtn && nav.previousTopic) {
      prevBtn.textContent = `← ${nav.previousTopic}`;
    }
    if (nextBtn && nav.nextTopic) {
      nextBtn.textContent = `${nav.nextTopic} →`;
    }
  }

  // ── 11. Sidebar Analytics ────────────────────────────────────────────────
  renderSidebarAnalytics(data) {
    const timeEl = document.getElementById(`${this.subject === 'history' ? 'hist' : this.subject}-sb-time`);
